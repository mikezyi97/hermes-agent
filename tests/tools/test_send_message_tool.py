...
class TestSendMessageTool:
    def test_cron_duplicate_target_is_skipped_and_explained(self):
        ...
        with patch.dict(
            ...
            clear=False,
             ...
             patch("gateway.session_context.get_session_env", side_effect=lambda name, default='': 'cron_test_run' if name == 'HERMES_SESSION_KEY' else default), \
    ...
    def test_cron_env_leak_does_not_skip_non_cron_session(self):
        home = SimpleNamespace(chat_id="-1001")
        ...
        with patch.dict(
            ...
            clear=False,
             ...
             patch("tools.send_message_tool._send_to_platform", new=AsyncMock(return_value={"success": True})) as send_mock, \
             patch("gateway.mirror.mirror_to_session", return_value=True) as mirror_mock, \
             patch("gateway.session_context.get_session_env", side_effect=lambda name, default='': 'agent:main:weixin:dm:o9cq8086' if name == 'HERMES_SESSION_KEY' else default):
            result = json.loads(
                send_message_tool(
                    {
                        "action": "send",
